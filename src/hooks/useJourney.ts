...
export interface Journey {
  ...
  /**
   * Where the rider is taken to be — the fix, or the last stop they confirmed
   * when the fix has stopped agreeing with them. See `believedPosition`.
   */
...
export function useJourney(
  ...
  const believed = useMemo(
    () => believedPosition(step, anchor, position),
    [step, anchor, position],
  );
  const progress = useMemo(() => stepProgress(step, believed), [step, believed]);
